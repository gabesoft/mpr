...
ClientLog.prototype.makeLabel = function(level) {
    return [
        new Text(levels[level].text, chalk[color](levels[level]))
...
ClientLog.prototype.list = function(procs, options) {
    ...
    procs.forEach(function(proc) {
        lines.push( [
          ...
          , new Text(util.str.rpad(proc.running ? '(' + proc.pid + ')' : '', pidLen), long ? 'green' : 'gray')
          , new Text(proc.running ? ' output ' : '')
          , new Text(proc.running ? (proc.off ? 'off' : 'on') : '', proc.off ? 'red' : 'green')
        ]);

        if (long) {
            lines.push(new Text(proc.command, 'gray'));
        }
    });
