...
Client.prototype.commandProc = function(mpr, command, fn) {
    if (mpr.argv.all) {
        ...
        this.command(mpr, opts, function(data) {
            // TODO: update log methods after refactoring logs
            //log[command](data.proc, data.status);

            log['client' + util.str.capitalize(command)](data.proc, data.status);
        });
    }
};
...
Client.prototype.ls = function(mpr) {
    ...
    this.command(mpr, opts, function(data) {
        ...
        log.ls(data.procs, data.status, { long: mpr.argv.long });
    });
};
